...
## [Canonical Only] 확정값 단일 사용 + 문제점·문안 완전 일치
> 최종 업데이트: 2026-09-11
> 적용 파일: `runtime/api/server.py`, `runtime/review/finding_integrity_gates.py`,
> `runtime/review/statute_applicability_gate.py`, `runtime/review/cross_type_template_gate.py`,
> `runtime/review/cross_clause_protection.py`, `runtime/review/accept_keep.py`

목표는 finding 수가 아니라 **정확도**다 — 우리 관점에서 정확한 쟁점만 남기고,
문제점과 수정문구가 완전히 일치해야 한다.

1. **canonical 재추론 금지** — 역할·계약유형·적용법률은 검토가 한 번 확정한 값만 쓴다.
   다운로드 경로가 `assess_statutes` 를 다시 돌리던 것을 `_statute_decisions_for()` 로
   바꿔 저장된 판단을 복원한다. 저장값이 없는 옛 세션만 새로 판단한다.
2. **비적용 법률 근거 전면 차단** — "주장하지 않으면 남긴다"(`_RX_STATUTE_ASSERTION`),
   "사실확인·누락 지적이면 남긴다"(`_RX_FACT_OR_GAP`) 두 예외를 제거했다.
   다만 **근거 차단과 위험 삭제는 다르다** — 계약 원문을 정규식으로 직접 확인한
   결정론적 rule 은 근거가 계약 문언이므로, finding 은 남기고 그 법률 근거 문장만
   떼어낸다(`_strip_statute_grounds`). 조문 문안은 훼손하지 않는다.
   (실측: 전략적 제휴계약의 설계·시공 위탁 공백 지적이 건설산업기본법 비적용
   판정으로 통째로 사라졌다. 계약상 공백은 그 법이 없어도 위험이다.)
3. **효과 불일치 즉시 hard fail** — 계약유형 whitelist 로 삭제 여부가 갈리던 것을
   전면 삭제로 통일했다. 종전의 완화는 "삭제하면 409로 다운로드가 막힌다"는 사정
   때문이었으나, 전달 게이트가 결함을 제거·기록한 뒤 전달하므로 그 사정이 없어졌다.
4. **타 유형 템플릿 혼입 시 결과 생성 금지** — 최종 자가점검이 감지만 하고 오염된
   finding 을 그대로 내보내던 것을, `cross_type_template_gate` 가 제거하도록 했다.
   계약이 스스로 쓰는 단어는 혼입이 아니다.
5. **중복 보호조항 HIGH/MEDIUM 금지** — 한 단계만 내려(HIGH→MEDIUM) 여전히
   권장수정에 남던 것을 LOW 까지 내린다. 삭제하지 않는 이유는 그대로다(정합성
   게이트가 금전 리스크 finding 의 존재를 검사한다). `cross_clause_suppressed` 가
   빈 배열로 고정되어 최종 게이트에 전달되지 않던 버그도 함께 고쳤다.
6. **유리·적법 조항 KEEP 확장** — 종전에는 "상대방 책임을 완화하는" 문형만 잡았다.
   `weakens_our_favorable_clause()` 가 우리 권리에 협의·기간·범위 제한을 **새로**
   붙이는 수정까지 KEEP 으로 돌린다. 단 **대칭 구조는 제외**한다 — 상호 NDA 는
   조항 문면상 "수신자가 부담" 이지만 그 수신자가 이번엔 우리다
   (`has_alternating_party_roles`, 계약 앞부분의 선언을 본다).
7. **UI/DOCX 단일 canonical** — UI 확정본이 있으면 다운로드 경로의 게이트 재적용을
   무효화한다(`_gate_targets`). 같은 게이트를 두 번 적용하는 것이 곧 재추론이며,
   그때 문서 본문이 화면과 어긋난다.

### 방향 판단 보강 (`clause_direction`)
KEEP 판단이 전부 방향에 기대므로 두 결함을 함께 고쳤다.
- `_BURDEN_VERB` 에 "자신의 책임과 비용으로", "책임·의무를 이행하여야" 문형 추가.
  실측(웹젠 물품공급 제4조): 우리가 부담자인 하자담보 조항이 "상대방 부담" 으로
  읽혀, 이를 제한하는 수정안이 KEEP 되고 HIGH finding 이 사라졌다.
