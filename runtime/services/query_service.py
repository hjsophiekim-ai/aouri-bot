...
TRIGGER_MAP = {
    ...
    # ACT-008/RISK-005: "하도급 단가감액" 규칙 — "하도급"이라는 단어 하나만
    # 있어도(제3자 하도급 시공사를 그저 언급하는 문장 등) 실제 단가 감액과
    # 무관하게 HIGH 승인필요 finding이 발동하던 실사례(2026-09-02, 전략적
    # 파트너십 계약의 "갑의 하도급 시공사에게"라는 무관한 문구가 트리거됨)
    # 이후 "하도급"을 단독 트리거에서 제거 — 이 규칙의 실질(단가 감액·인하)
    # 을 직접 가리키는 문구만 트리거로 남긴다. contract_type 필터(공사/도급/
    # 하도급, 물품공급/구매/매매)는 그대로 유지되어 관련 없는 계약유형에는
    # 여전히 적용되지 않는다.
    "ACT-008": ["단가 감액", "단가 인하", "단가감액", "단가인하", "price reduction"],
    ...
    "RISK-005": ["단가 감액", "단가 인하", "단가감액", "단가인하", "price reduction"],
