REVIEW_RESULTS_HTML = """<!doctype html>
<html lang="ko">
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <title>AouriBot Review Results (Read-only)</title>
  <style>
...
<body>
  <h1>검토 결과 조회 (Read-only)</h1>
  <div class="muted"><a href="/admin">Rules Admin</a> | <a href="/upload">Upload</a></div>

  <div class="row">
    <input id="fEntity" placeholder="법인(entity) 필터" />
    <input id="fType" placeholder="계약유형(contract_type) 필터" />
    <label><input type="checkbox" id="fHighRisk" /> high risk만</label>
    <label><input type="checkbox" id="fApproval" /> approval required만</label>
  ...
  <div class="grid">
    <div class="panel">
      ...
      <table>
        <thead>
          <tr>
            <th>created_at</th><th>entity</th><th>contract_type</th><th>risk</th><th>approval</th><th>request_id</th>
          </tr>
        </thead>
    ...
    <div class="panel">
      ...
      <div class="row">
        <span class="badge" id="badgeRules"></span>
      ...
      <h3>Applied Rules</h3>
      <table>
        <thead><tr><th>rule_id</th><th>status</th><th>risk</th><th>approval</th><th>expanded</th><th>title</th></tr></thead>
  ...
  <script>
    async function loadList() {
      ...
      const url = `/api/reviews?limit=50&offset=0&entity=${entity}&contract_type=${contract_type}&high_risk_only=${high_risk_only}&approval_required_only=${approval_required_only}`;
    ...
    async function loadDetail(requestId) {
      ...
      document.getElementById('detailMeta').innerText =
        `request_id=${data.request.request_id} filename=${data.request.filename || ''}`;
      document.getElementById('badgeRules').innerText =
        `rules_sha256=${(data.rules_version && data.rules_version.rules_sha256) ? data.rules_version.rules_sha256.slice(0, 12) : 'n/a'}`;
      document.getElementById('badgeCounts').innerText =
        `high_risk=${data.result.high_risk_count} approval=${data.result.approval_required_count}`;
      ...
      for (const it of data.applied_rules || []) {
        const tr = document.createElement('tr');
        tr.innerHTML = `<td>${it.rule_id}</td><td>${it.rule_status}</td><td>${it.risk_level}</td><td>${it.approval_required}</td><td>${it.context_expanded_by_questions}</td><td>${it.title}</td>`;
        rulesBody.appendChild(tr);
      }

      document.getElementById('rawOut').innerText = JSON.stringify(data.result.raw, null, 2);
    }
