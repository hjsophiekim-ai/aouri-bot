...
_REGEX_FORMAL: list[tuple[str, str]] = [
    # "즉시"는 즉시해지/즉시종료(불이익 조기 발동) 맥락에서만 시정절차
    # 부재가 문제이므로 최고 절차를 넣는다. 예전에는 "즉시"라는 단어가
    # 나오면 문맥과 무관하게 무조건 치환했는데(단순 str.replace), 그
    # 결과 "그 사유를 알게 된 즉시 갑에게 통지" 같은 통지의무 문장까지
    # "상당한 기간을 정하여 최고한 후 통지"로 바뀌어 불가항력/공급망
    # 차질 사유는 애초에 최고(형식적 시정 요구)할 대상이 아닌데도
    # 그런 절차가 삽입되는 오류가 있었다(마코디 전략적 파트너십 계약
    # 제12조④ 검토에서 확인). 해지/해제/종료 바로 앞의 "즉시"만 치환한다.
    (r"즉시\s*(해지|해제|종료)", r"상당한 기간을 정하여 최고한 후 \1"),
    (r"하고\s*있다\b", "하고 있다"),
    (r"되어\s*있다\b", "되어 있다"),
    (r"([가-힣])\s*이고\b", r"\1이며"),
    (r"([가-힣])\s*이지만\b", r"\1이나"),
