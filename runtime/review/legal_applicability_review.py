...
_KNOWN_STATUTES: dict[str, list[str]] = {
    ...
    # 2026-09-04 지시(그림닷컴 판매지원 용역계약 실사례) — 판매/위탁판매/
    # 대리판매/중개 계약에서 소비자 대면 판매·창작물 판매가 있으면 실제로
    # 문제될 수 있는데 기존 사전에 없어 사용자가 명시적으로 언급해도
    # 독립 분석(guaranteed analysis) 대상이 되지 못했다.
    "전자상거래법": ["전자상거래법", "전자상거래 등에서의 소비자보호에 관한 법률", "전자상거래등에서의소비자보호에관한법률"],
    "소비자기본법": ["소비자기본법", "소비자보호법"],
    "저작권법": ["저작권법"],
}
...
_STATUTE_SPECIFIC_GUIDANCE: dict[str, str] = {
    # 2026-09-04 지시(그림닷컴 판매지원 용역계약 실사례) — 대리점법을 "상대방이
    # 자기 명의로 재판매하지 않는다"는 이유만으로 낮음으로 판정하는 오류를
    # 막는다. 대리점법은 재판매뿐 아니라 위탁판매 구조도 대상으로 한다.
    "대리점법": (
        "대리점법(대리점거래의 공정화에 관한 법률) 적용 여부는 상대방이 자기 명의로 "
        "재판매하는지 여부만으로 판단하지 마십시오 — 대리점법은 재판매뿐 아니라 위탁판매 "
        "구조(소유권이 공급업자에게 남아있고 대리점은 판매를 대행·지원하는 구조)도 "
        "적용 대상으로 합니다. 다음 순서로 종합 판단하십시오: (1) 당사자 사이에 기존 "
        "공급업자-대리점(위탁판매) 관계가 이미 존재하는지, (2) 이번 계약(또는 조항)의 "
        "업무가 그 기존 대리점관계에 부수·연계되는 것인지 — 별도 계약 형식이더라도 "
        "동일 매장·인력·POS(결제 단말기)를 사용하거나 계약기간·해지가 기존 계약에 "
        "연동되어 있으면 경제적·운영상 연계로 봅니다, (3) 대리점이 추가 업무 참여를 "
        "자율적으로 선택했는지(자율적 참여 및 불이익 금지 조항이 있는지), (4) 참여를 "
        "거절하거나 종료할 경우 기존 대리점계약상 불이익이 있는지, (5) 추가로 부담하는 "
        "인력·공간·비용에 상응하는 적정 보수가 지급되는지. (1)과 (2)가 확인되면, "
        "위탁판매/재판매 여부와 무관하게 최소 '있음(추가 확인 필요)'(risk_level MEDIUM "
        "이상)으로 판단하고, (3)~(5) 중 불리한 사실(자율성 부재, 불이익 존재, 부적정 "
        "보수)이 확인될수록 위험도를 높이십시오. 기존 대리점관계 자체가 없다면(완전히 "
        "독립된 신규 거래) 적용 가능성은 낮을 수 있습니다."
    ),
    "하도급법": (
        ...
        "여부)가 확인되는지. 고객 상담·전시·주문접수·결제대행·배송안내·재고관리 같은 "
        "단순 판매지원 업무는 그 자체로 하도급법상 용역위탁에 해당하지 않습니다 — "
        "이런 업무만 확인된다면 risk_level을 MEDIUM 이상으로 자동 승격하지 말고 "
        "'낮음'/LOW로 판단하십시오. 위 (1)~(3) 중 어느 것에도 해당하지 않고 단순 "
        "판매지원·소개·연계에 그친다면 하도급법 적용 가능성은 낮습니다. 실제로 제조·"
        "설계·시공·수리 위탁에 해당하는 사실관계가 있는데 세부사항이 불명확한 경우에만 "
    ...
    "전자상거래법": (
        "전자상거래법(전자상거래 등에서의 소비자보호에 관한 법률) 적용 여부는 다음을 먼저 확인하십시오: "
        "(1) 최종 소비자와의 매매계약상 실제 판매자(매도인)가 누구인지 — 매장 POS/결제 단말기의 "
        "명의자를 곧바로 판매자로 단정하지 마십시오(위탁매매 구조에서는 다를 수 있음), (2) 통신판매업 "
        "신고 대상 거래에 해당하는지, (3) 청약철회·환불 의무를 실제로 이행하는 주체가 누구인지. 판매자가 "
        "불명확하면 '적용 가능성 있음(추가 확인 필요)'로 판단하고 판매자 확정에 필요한 사실관계를 "
        "구체적으로 나열하십시오."
    ),
    "소비자기본법": (
        "소비자기본법/소비자보호 관련 책임은 계약당사자 사이의 문제가 아니라 최종 소비자에 대한 책임 "
        "문제입니다 — 계약서에 하자·반품·환불 책임 주체가 명시되어 있는지, 명시되어 있지 않다면 실제로 "
        "소비자를 직접 응대하는 당사자가 표현상 책임을 떠안을 위험이 있는지를 판단하십시오."
    ),
    "저작권법": (
        "작품·창작물 판매·전시·홍보가 있는 계약에서는 (1) 저작권/진위 보증 책임이 누구에게 있는지, "
        "(2) 작가명·이미지의 홍보·SNS 사용권이 명시되어 있는지, (3) 침해 클레임 발생 시 배상 책임 "
        "주체가 누구인지를 확인하십시오. 계약서에 아무 규정이 없다면 '확인 필요'로 판단하십시오."
    ),
}
...
_STRUCTURAL_STATUTE_SIGNALS: dict[str, list[str]] = {
    ...
    "대리점법": ["대리점", "재판매가격", "판매장려금", "판촉비", "위탁판매", "위탁운영", "대리판매"],
    "하도급법": ["위탁", "하도급", "재위탁"],
    "전자상거래법": ["고객", "판매", "결제", "환불", "반품", "배송"],
    "소비자기본법": ["고객", "소비자", "하자", "환불", "반품"],
    "저작권법": ["저작물", "저작권", "작품", "그림", "이미지 사용"],
}
...
입력의 "contract_legal_map"에 seller, owner_of_goods, payment_recipient,
revenue_recipient, existing_related_contract 같은 필드가 이미 채워져 있다면,
이는 사용자가 사전질문에 직접 답변해 확정한 사실관계입니다. 계약서 원문에
이 내용이 명시적으로 나오지 않더라도 이 값을 그대로 확정된 사실로 전제하고
판단하십시오 — 판매자·소유자·기존 거래관계를 다시 추정하거나 "확인 필요"로
되돌리지 마십시오.
...
한국 법률의 적용 가능성은 당사자가 한국 법인이라는 사실만으로 자동으로
높다고 판단하지 마십시오. 반드시 먼저: (1) 이 계약에 따른 서비스/업무가
실제로 수행되는 국가·지역(주)이 어디인지, (2) 실제 근로자·인력이 소재한
곳이 어디인지, (3) 개인정보가 실제로 어디서 수집·처리되는지, (4) 그 업무가
현지(외국)의 별도 규제 대상 업무인지를 계약 내용에서 확인하십시오. 예를
들어 한국 회사가 당사자이더라도 미국 캘리포니아주 현지 직원을 대상으로 한
HR 아웃소싱처럼 실제 수행지·근로자·개인정보가 전부 외국에 있다면, 한국
파견법·한국 개인정보보호법을 자동으로 핵심 적용법률로 올리지 말고 대신
현지(예: California/미국 연방) 고용법·개인정보보호법을 우선 검토 대상으로
제시하십시오. 한국 법률은 실제로 한국과의 연결점(한국 내 근로·한국에서
수집된 개인정보 등)이 있을 때만 추가하십시오. "HR 업무이니 파견법",
"직원 정보이니 한국 개인정보보호법"처럼 업무 종류만 보고 자동으로 특정
한국 법률에 매핑하는 것은 금지합니다.

적용법률 판단 우선순위는 반드시 다음 순서를 따르십시오: ① 실제 업무
수행지(계약상 서비스가 실제로 수행되는 국가·지역), ② 규제 대상이 되는
사람·자산의 실제 소재지(근로자·개인정보·물건 등이 실제로 어디에 있는지),
③ 실제 규제행위(그 법률이 규율하는 행위가 실제로 계약에 존재하는지),
④ 계약서에 명시된 준거법 조항. 당사자의 국적이나 계약서에 등장하는
법률 관련 키워드만으로 이 순서를 건너뛰고 결론을 내리지 마십시오 —
준거법 조항은 이 네 가지 중 마지막 참고자료일 뿐, 그 자체가 실제
연결점을 만들어내지 않습니다.

법률을 목록에 추가할 때는 다음 세 가지가 모두 계약 내용에서 확인되는
경우에만 "있음(추가 확인 필요)" 이상(risk_level MEDIUM 이상)으로
판단하십시오: ① 실제 연결점(actual nexus — 관할·소재지·근로지 등이 그
법률의 관할과 실제로 닿아 있는지), ② 실제 규제대상 행위(actual regulated
activity — 그 법률이 규율하는 행위 유형이 실제로 계약에 있는지), ③ 실제
관할(actual jurisdiction — 그 법률의 관할권이 이 거래에 미치는지). 셋 중
하나라도 단순한 가능성 수준에 그치고 확인되지 않으면 risk_level을 LOW로
낮추거나 additional_facts_needed에 "확인 필요" 사실관계로만 표시하고,
applicability를 "낮음" 또는 "있음(추가 확인 필요)"으로 낮춰 답하십시오.
