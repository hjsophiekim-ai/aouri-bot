...
_BURDEN_VERB = (
    ...
    r"|보증(?:한다|하며|하여야)"
    r"|면책(?:시킨다|하며|하여야)"
    # 실측(웹젠 물품공급 제4조): "'을'은 … 자신의 책임과 비용으로 … 하자담보
    # 책임을 이행하여야 하며" — 우리가 부담자인 전형적 문형인데 위 목록에
    # 걸리지 않아, 같은 조항의 뒷문장에 있던 다른 부담 문형이 잡히면서 방향이
    # 상대방 부담으로 뒤집혔다. 이행·자기비용 문형을 함께 본다.
    r"|자신의\s*(?:책임과\s*)?비용으로"
    r"|(?:책임|의무)(?:를|을)?\s*이행하(?:여야|며|고|기로)"
    r"|(?:수리|교환|환불|반품)(?:를|을)?\s*(?:하여야|이행))"
)
...
#: 주격 조사처럼 끝나지만 주어가 아닌 접속·부사 어절. "또는"의 "또", "및"의
#: 앞부분처럼 조사 분리에 걸려 주어로 잡히면, 문장의 진짜 주어를 가린다.
#: 실측: `“갑” 또는 사용자의…` 에서 "또"가 첫 주어로 잡혔다.
_RX_NOT_A_SUBJECT = re.compile(
    r"^(?:또|또한|그리고|및|한편|다만|단|즉|따라서|그러나|하지만|그런데"
    r"|만약|만일|아울러|나아가|반면|오히려|특히|비록)$"
)
...
def _effective_subject(body: str, verb_start: int) -> str:
    ...
    for m in _RX_SUBJECT_TOKEN.finditer(body, start, verb_start):
        ...
        if _RX_NOT_A_SUBJECT.match(candidate):
            continue
        subject = m.group(1)
        break
    return subject
