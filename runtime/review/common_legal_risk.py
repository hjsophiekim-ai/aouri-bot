...
_RX_LATE_PENALTY_RATE = re.compile(
    ...
    # 표준 공사도급계약 서식(2026-09-09 실측) — "매 지체일수마다 계약서상의
    # 지체상금률(1/1000)을 계약금액에 곱하여 산출한 금액". 요율이 괄호 안
    # 분수로 오고 기준 금액이 **뒤에** 나오는 형태라, 앞의 갈래들이 요구하는
    # "지체일수 당 <금액명사>의 N/M" 어순과 어긋나 전부 놓쳤다. group 7/8.
    r"|지체상금률\s*[（(]\s*(\d+(?:\.\d+)?)\s*/\s*(\d+)\s*[)）]",
    re.IGNORECASE | re.DOTALL,
)
...
def _apply_late_penalty_uncapped_check(
    ...
    for c in (clauses or []):
        ...
        if m.group(1) and m.group(2):
            ...
            # "지체상금률(1/1000)" -> 0.1%
            rate_pct = float(m.group(7)) / float(m.group(8)) * 100
        else:
            continue
