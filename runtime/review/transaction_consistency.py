...
#:
#: [2026-09-11 지시] "바터계약에 현금 대금 지급·보류 템플릿 삽입 금지."
#: 지급 자체뿐 아니라 **지급 보류·상계·감액** 도 현금 대가가 있어야 성립한다.
#: 무현금 교환에서 "대금 지급을 보류할 수 있다" 는 보류할 대상이 없는 문구다.
_RX_CASH_PAYMENT_TEMPLATE = re.compile(
    ...
    # 지급 보류·상계·감액 계열
    r"|(?:대금|대가|잔금)(?:의)?\s*(?:지급)?\s*(?:유보|보류)"
    r"|지급(?:을|의)?\s*(?:유보|보류)(?:할\s*수\s*있|한다|하며)"
    r"|(?:대금|대가)(?:에서)?\s*(?:상계|공제|차감)(?:할\s*수\s*있|한다)"
    r"|(?:대금|대가)(?:을|를)?\s*감액"
    r"|미지급(?:금|액)|지급기일\s*다음\s*날",
    re.IGNORECASE,
)
