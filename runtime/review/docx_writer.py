...
def _r(
    ...
    *,
    ...
    rpr = None
    if bold or underline or (isinstance(color_hex, str) and color_hex.strip()) or (isinstance(highlight, str) and highlight.strip()):
...
def build_revision_docx(
    *,
    ...
    title = "아우리봇 계약서 수정본(REDLINE 스타일, MVP)"
    meta = [
        f"법인: {entity}",
    ...
    sec1 = _p(body)
    _t(_r(sec1, bold=True), "1) 원문 계약서 (Clean Copy)")
    for c in original_clauses:
        if not isinstance(c, dict):
            continue
        cid = str(c.get("clause_id") or "")
        ctitle = str(c.get("clause_title") or "")
        head = (cid + " " + ctitle).strip() if (cid or ctitle) else "조항"
        ph = _p(body)
        _t(_r(ph, bold=True), head)
        for line in _split_lines(str(c.get("text") or ""))[:120]:
            pl = _p(body)
            _t(_r(pl), line)
        _p(body)

    sec2 = _p(body)
    _t(_r(sec2, bold=True), "2) 수정본 (수정 조항 강조)")
    for c in original_clauses:
        if not isinstance(c, dict):
            continue
        cid = str(c.get("clause_id") or "")
        ...
        cr = cr_by_id.get(cid)
        if cr and isinstance(cr.get("suggested_rewrite"), str) and str(cr.get("suggested_rewrite") or "").strip():
            sr = str(cr.get("suggested_rewrite") or "")
            for line in _split_lines(sr)[:120]:
                pl = _p(body)
                _t(_r(pl, bold=True, underline=True, color_hex="D64545", highlight="yellow"), line)
            rr = cr.get("rewrite_reason")
            prh = _p(body)
            _t(_r(prh, bold=True), "수정 이유:")
            if isinstance(rr, str) and rr.strip():
                for line in _split_lines(rr)[:60]:
                    pl = _p(body)
                    _t(_r(pl), line)
            else:
                pl = _p(body)
            ...
            original_text = str(c.get("text") or "")
            for line in _split_lines(original_text)[:120]:
                pl = _p(body)
                _t(_r(pl), line)
        _p(body)

    sec3 = _p(body)
    _t(_r(sec3, bold=True), "3) 부록: 조항별 원문/수정문/이유 표")
    tbl = _tbl(body, col_widths=[1600, 3000, 3000, 2400])

    def add_row(cells: list[list[tuple[str, dict[str, Any]]]]) -> None:
        tr = ET.SubElement(tbl, _w("tr"))
        for i, paras in enumerate(cells):
            tc = _tc(tr, width=int([1600, 3000, 3000, 2400][i] if i < 4 else 2000))
    ...
    for c in original_clauses:
        ...
        orig_lines = _split_lines(str(c.get("text") or ""))[:20]
        orig_paras = [(x, {}) for x in orig_lines] if orig_lines else [("", {})]

        cr = cr_by_id.get(cid)
        sr = str(cr.get("suggested_rewrite") or "") if isinstance(cr, dict) else ""
        rr = str(cr.get("rewrite_reason") or "") if isinstance(cr, dict) else ""
        sr_paras = [(x, {"bold": True, "underline": True, "color_hex": "D64545"}) for x in _split_lines(sr)[:20]] if sr.strip() else [("", {})]
        rr_paras = [(x, {}) for x in _split_lines(rr)[:20]] if rr.strip() else [("", {})]
        add_row([[ (left, {}) ], orig_paras, sr_paras, rr_paras])
