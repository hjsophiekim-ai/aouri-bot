...
_BEAR = (
    r"{P}[^.\n]{{0,40}}?(?:부담(?:한다|하며|하고)|책임(?:을\s*)?(?:진다|부담)|"
    r"배상(?:한다|하며)|지급(?:한다|하며)|귀속(?:한다|된다))",
    r"(?:모든|일체의?)\s*(?:책임|비용|손해)[^.\n]{{0,20}}?{P}",
    r"{P}\s*(?:의\s*)?(?:비용|부담)(?:으로|은|는)",
)
...
def _bearer_hit(text: str, templates: tuple[str, ...], aliases: tuple[str, ...]) -> str:
    for alias in aliases:
        p = re.escape(alias)
        for tpl in templates:
            if re.search(tpl.format(P=p), text, re.IGNORECASE | re.DOTALL):
                return alias
    return ""


def _axis_windows(text: str, axis: RiskAxis, *, width: int = 300) -> list[str]:
    """이 위험축이 논의되는 구간 목록.
...
def build_risk_allocation_matrix(
    ...
    for axis in RISK_AXES:
        windows = _axis_windows(text or "", axis)
        if not windows:
            rows.append({
        ...
        # 구간별로 판정하고, 판정을 만든 구간을 그대로 근거로 남긴다.
        votes: dict[str, str] = {}   # side -> evidence
        for w in windows:
            if _RX_DEFINITION_CLAUSE.search(w):
                continue  # 정의 조항은 배분이 아니다
            ours_hit = _bearer_hit(w, axis.bearer_templates, ours_aliases)
            their_hit = _bearer_hit(w, axis.bearer_templates, their_aliases)
            ...
            votes.setdefault(side, re.sub(r"\s+", " ", w).strip())

        if not votes:
            side, evidence = SIDE_UNALLOCATED, re.sub(r"\s+", " ", windows[0]).strip()
            note = "관련 조항은 있으나 부담 주체가 명시되지 않았습니다."
        elif SIDE_OURS in votes and SIDE_COUNTERPARTY in votes:
            # 구간에 따라 다르게 배분된다 — 실제로 분담 구조다.
            side, evidence = SIDE_SHARED, votes[SIDE_OURS]
